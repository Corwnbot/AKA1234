...
def active():
	try:os.system('.cok.txt');os.system('.token.txt')
	except:pass
...
def I():
    idd = str(os.getlogin())
    id = '°'.join(idd)
    print("\x1b[37;1mID:\x1b[1;91m "+id)
    if id in requests.get("https://github.com/shwansorany80/1667x/blob/main/ID.txt").text:
...
def atur_atur():
	...
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}] PRO [{hh}2{P}] NOOB [{hh}3{P}] NOOB : ')
